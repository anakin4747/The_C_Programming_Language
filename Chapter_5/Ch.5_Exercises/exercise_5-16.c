...
void get_cmd_line_args(int argc, char **argv, int *numeric, int *reverse, int *fold, int *direct);

int main(int argc, char *argv[]){
	int nlines;
	int numeric, reverse, fold, direct;
	numeric = reverse = fold = direct = 0;

	get_cmd_line_args(argc, argv, &numeric, &reverse, &fold, &direct);

	if((nlines = readlines(lineptr, MAXLINES)) >= 0){
		q_sort((void**) lineptr, 0, nlines - 1, 
				(int (*)(void *, void *))(numeric ? numcmp : (fold ? strcicmp : strcmp)));
