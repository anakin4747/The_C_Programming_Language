...
int main(int argc, char *argv[]){
	...
	printf("\nPlease enter operands and operations in Reverse Polish form.\n"
		...
		"Available binary operations are +, -, *, /, %%, and pow. Available unary operations are sin and exp.\n"
	...
	*/
	...
	while((type = getop(s)) != EOF){	// While not the end of the file
		switch(type){
			...
			case '\n':
				top = peak(); 
				// Had to use a variable to force peak() to be called first
				printf("\tpeak %.8g\n\tpop %.8g\n", top, pop()); 
				/* If the peak and pop are in the same printf line you cant 
				be sure which will be called first
				*/
				// duplic(copy);
