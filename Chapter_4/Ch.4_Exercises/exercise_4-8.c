...
int getop(char s[]){ // Gets next char or numeric operand
	...
	if(isalpha(c)){
        ...
        s[i] = '\0';
        ungetch(c);
        ungetch(c); // Use any of these to test the single char buffer
		if(!strcmp(s, "sin")){
			return SIN_FOUND;
...
char buf;	// Buffer
int bufp = 0; // Indicates if the value in the buffer is valid or not as being empty or full	
...
void ungetch(int c){
	if(bufp)	// If buffer has reached capacity
		printf("ungetch: too many characters\n");// Print error
	else{
		buf = c; // Add c to the next free position in the buffer
		bufp = 1;	// Increment buffer size to reflect increase
    }
}
