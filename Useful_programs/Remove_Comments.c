...
int main(int argc, char *argv[]){
    int c, c2, old_c = 0, sl_comment, ml_comment, quote;
    ...
    if((new_file_ptr = fopen("temp.c", "w")) == NULL){
    ...
    while((c = fgetc(old_file_ptr)) != EOF){    // While the old file is not at the end
        ...
        // Main logic for copying characters to new file
        if((sl_comment == OUT && ml_comment == OUT) || quote == IN){ // If you do not read a '/' and are outside a comment or are in a quote 
            c2 = fgetc(old_file_ptr);   // Save the next char
            if(c == '/' && (c2 != '/' && c2 != '*') && (old_c != '/' && old_c != '*')){ // If c is a slash and the next one isnt 
                fputc(c, new_file_ptr);     // Copy the slash from old file to new
            } else if(c != '/')         // Else if c isnt a slash
                fputc(c, new_file_ptr);     // Copy char from old file to new
            ungetc(c2, old_file_ptr);   // Put back next char
        }
