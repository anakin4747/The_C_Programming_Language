...
int main(int argc, char *argv[]){
    int next_c, c, old_c = 0, sl_comment, ml_comment, quote;
    ...
    while((next_c = fgetc(old_file_ptr)) != EOF){    // While the old file is not at the end
        ...
        else if((c == '/' && next_c != '/' && sl_comment == OUT && ml_comment == OUT) || quote == IN)
            fputc(c, new_file_ptr);     // Copy char from old file to new
        if(next_c == EOF)
